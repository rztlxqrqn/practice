  ...
  describe('edge parse',()=>{
    it('one edge',()=>{
      ...
      return graph.parseEdge('A','B',5)
        .then((gragh)=>{
          ...
          expect(Object.keys(gragh.nodes[0].directNextNodes).length).to.equal(1);
          expect(gragh.nodes[0].directNextNodes['B']).to.equal(5);
    ...
    it('three edges',()=>{
      ...
      return graph.parseEdge('A','B',5)
        .then((graph)=>{
          expect(graph.nodes.length).to.equal(2);
          expect(graph.nodes[0].name).to.equal('A');
          expect(Object.keys(graph.nodes[0].directNextNodes).length).to.equal(1);
          expect(graph.nodes[0].directNextNodes['B']).to.equal(5);
        ...
        .then((graph)=>{
          return graph.parseEdge('A','C',4)
            .then((graph)=>{
              ...
              expect(Object.keys(graph.nodes[0].directNextNodes).length).to.equal(2);
              expect(graph.nodes[0].directNextNodes['B']).to.equal(5);
              return graph;
            })
        })
        .then((graph)=>{
          return graph.parseEdge('C','B',3)
            .then((graph)=>{
              ...
              expect(Object.keys(graph.nodes[0].directNextNodes).length).to.equal(2);
              expect(graph.nodes[0].directNextNodes['B']).to.equal(5);
              expect(graph.nodes[0].directNextNodes['C']).to.equal(4);
              expect(graph.nodes[2].directNextNodes['B']).to.equal(3);
              return graph;
            })
        })
    });
    ...
    it('repetition error',()=>{
      ...
      return graph.parseEdge('A','B',5)
        ...
        .catch((err)=>{
          expect(err.message).to.equal('nextNode duplicate');
        })
    });
    it('repetition update',()=>{
      ...
      return graph.parseEdge('A','B',5)
        ...
        .then((graph)=>{
          expect(graph.nodes[0].directNextNodes['B']).to.equal(4);
        })
    });
  });
  describe('getNode',()=>{
